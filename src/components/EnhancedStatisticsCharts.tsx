...
export function EnhancedStatisticsCharts({ promises }: EnhancedStatisticsChartsProps) {
  ...
  const statusData = promises.reduce((acc, promise) => {
    ...
    if (existing) {
      ...
      acc.push({
        ...
        value: 1,
        name: STATUS_LABELS[promise.status],
      });
    }
    return acc;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      {/* Party Comparison Bar Chart */}
      <Card className="p-6">
        <h3 className="text-lg font-semibold mb-4">Jämförelse mellan partier</h3>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={partyChartData}>
            ...
            <Bar dataKey="Delvis infriade" stackId="a" fill="hsl(var(--chart-2))" />
