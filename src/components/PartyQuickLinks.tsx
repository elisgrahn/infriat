  ...
  return (
    <nav aria-label="Snabblänkar till partierna" className="mb-4">
      <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1.5">
        <span className="text-xs text-muted-foreground mr-1">Snabbt till ett parti:</span>
        {PARTY_ORDER.map((abbr) => (
          <Link
            key={abbr}
            to="/parti/$kort"
            params={{ kort: abbr }}
            className="inline-flex items-center gap-1 rounded-full border bg-card px-2.5 py-1 text-xs font-medium text-foreground transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
            <span
            ...
            <span className="hidden lg:inline">{PARTY_ABBREVIATION_TO_NAME[abbr]}</span>
