        ...
        >
          {parties.map((party) => (
            <ToggleGroupItem
              key={party}
              value={party}
              className={`transition-all text-sm data-[state=on]:text-gray-300 ${partyColors[party]}`}
            >
              {party}
            </ToggleGroupItem>
          ))}
        ...
        >
          {statuses.map((status) => (
            <ToggleGroupItem
              key={status}
              value={status}
              className={`transition-all text-sm data-[state=on]:text-gray-300 ${statusColors[status]}`}
            >
              {status}
            </ToggleGroupItem>
          ))}
